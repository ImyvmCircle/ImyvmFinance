...
public final class SimulationFormula {
    public static final String DEFAULT = "CLAMP(DRIFT_BPS + VOLATILITY_BPS * RANDOM, -MAX_MOVE_BPS, MAX_MOVE_BPS)";
    ...
    public static SimulationFormula parse(String source) {
        ...
        double sample = root.eval(Map.of("PREV_PRICE",10000d,"PREV_LOG_RETURN",0d,"DRIFT_BPS",10d,"VOLATILITY_BPS",20d,"MAX_MOVE_BPS",100d,"RANDOM",.25d,"ITERATION",1d,"HISTORY_COUNT",5d));
        if (!Double.isFinite(sample)) throw new IllegalArgumentException("formula result is not finite");
    ...
    private static final Set<String> VARIABLES=Set.of("PREV_PRICE","PREV_LOG_RETURN","DRIFT_BPS","VOLATILITY_BPS","MAX_MOVE_BPS","RANDOM","ITERATION","HISTORY_COUNT");
    private static final Set<String> FUNCTIONS=Set.of("LN","LOG10","LOG2","LOGN","EXP","POW","SQRT","ABS","SIGN","MIN","MAX","CLAMP","FLOOR","CEIL","ROUND");
    private static final class Parser { final String s; int i; Parser(String s){this.s=s;} void skip(){while(i<s.length()&&Character.isWhitespace(s.charAt(i)))i++;} boolean end(){return i>=s.length();} boolean take(char c){if(i<s.length()&&s.charAt(i)==c){i++;return true;}return false;} void need(char c){skip();if(!take(c))throw error("expected "+c);} IllegalArgumentException error(String m){return new IllegalArgumentException(m+" at "+i);}
